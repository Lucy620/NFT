...
function Home () {
  ...
  const buyToken = async(tradeId, price,amount) => {  
   ...
   const bidToken = async(tradeId, price) => {  
	...
	var balance = await blockchain.web3.utils.fromWei(blockchain.web3.eth.getBalance(blockchain.account),'ether');
	console.log(balance);
	console.log(price);

	if(await blockchain.web3.utils.fromWei(balance,'ether') > await blockchain.web3.utils.fromWei(price,'ether')){
		console.log(price);
