    ...
    const fetchNFT = async () => {
        ...
        try {
          ...
          await fetch(nftInfo.uri).then((response) => response.json()).then((metaData) => {
            setNFT([
              { id:nftInfo._tradeId, metaData:metaData, poster:nftInfo.poster, price:nftInfo.price, amount:nftInfo.amount, auctionType:nftInfo.auction}
            ]);
          }).catch((err) => {
            console.log(err);
          });
        ...
        {nft.map((item,index) =>(
           ...
           <s.rightBox className="top24" ai={"right"}>
			<div style={{width:'500px'}}>
				...
				<div className="top12 size12 italic">#{item.id}</div>
