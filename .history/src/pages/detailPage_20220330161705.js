  ...
  if (completed) {
    ...
    const fetchNFT = async () => {
        ...
        try {
          ...
          await fetch(nftInfo.uri).then((response) => response.json()).then((metaData) => {
            setNFT([
              { id:nftInfo._tradeId, metaData:metaData, poster:nft.poster,price:nftInfo.price, amount:nftInfo.amount, auctionType:nftInfo.auction}
            ]);
          }).catch((err) => {
            console.log(err);
          });
    ...
    const buyToken = async(tradeId, price,amount) => {  
     ...
     const bidToken = async(tradeId, price) => {  
      let time = moment().format('YYYY-MM-DD HH:mm:ss');  
      if(price > processingPrice){
		blockchain.MarketPlace.methods.bid(tradeId, time)
		.send({from: blockchain.account, value: price})
		.once("error", (err) => {
		 alert('failed!');
		 console.log(err);
		}).then(() => {
		 ...
		 fetchAuction();
		   ...
		   alert("The price need to over current highest bid");
	   }
        ...
        {nft.map((item,index) =>(
           ...
           <s.rightBox className="top24" ai={"right"}>
			<div style={{width:'500px'}}>
				...
				<div className="top12 size12 color5">Owned by <span className="color-blue bold">NFTKingCreator</span></div>
					...
					{(auction.beneficiary != 0 && auction.highestBid) ? (
						...
						<>	
						   <div className="size12 italic color5">current price:</div>
							<div className="top8 flex italic ">
								<img src={ehtIcon} className="icon16"/>
								<div className="size18 bold"><div className="size18 italic bold">{blockchain.web3.utils.fromWei(item.price,'ether')} ETH</div></div>
							</div>
							<div className="top8">
			...
			<Modal
			  ...
			  onOk={(e) => {
			         form
			           ...
			           .then(values => {
							handleClose1();
							bidToken(processingID, values.price);
			           })
			           .catch(info => {
			         ...
			         form
			           ...
			           .then(values => {
						   ...
						   console.log(values.amount)
							handleClose2();
							buyToken(processingID, processingPrice,values.amount);
			           })
			           .catch(info => {
			             console.log('Validate Failed:', info);
			           });
