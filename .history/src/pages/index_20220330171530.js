...
function Home () {
  ...
  const buyToken = async(tradeId, price,amount) => {  
   ...
   const bidToken = async(tradeId, price) => {  
	...
	console.log(balance);
	console.log(blockchain.web3.utils.toWei(processingPrice.toString(),'ether'));
	if(price > blockchain.web3.utils.toWei(processingPrice.toString(),'ether') && balance > price){
     ...
     .once("error", (err) => {
	  ...
	  alert('success!');	
		...
		alert("The price not over current highest bid or not enough balance")
	}
